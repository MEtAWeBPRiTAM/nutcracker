...
async def availableBots(bot, message):
    ...
    keyboard = [
        [InlineKeyboardButton(bot[0], url=bot[1])] for bot in bot_list
    ]
...
async def handleMessage(bot, message):
    ...
    if video_links:
        messageInit = await bot.send_message(message.chat.id, 'Processing request... 👍')
...
async def handleImage(bot, message):
    ...
    if caption:
        ...
        if video_links:
            ...
            await bot.send_chat_action(message.chat.id, 'typing')
            for video_link in video_links:
                localFilePath = os.path.join(f'../public/uploads', f'{os.path.basename(video_link)}')
                unique_link = await process_video_link(video_link, user_id, sender_username, localFilePath, uniqueId)
                await message.reply(f'''Your video has been uploaded successfully... \n\n😊😊Now you can start using the link:\n\n{unique_link}''')
                await messageInit.delete()
        else:
